        (() => {
            ...
            function drawTitleScreen() {
                ...
                ctx.fillStyle = '#fff';
                ctx.font = '18px sans-serif';
                ctx.fillText('Logros / Achiev.', canvas.width / 2, 550);
                ...
                for (let i = pinatas.length - 1; i >= 0; i--) {
                    ...
                    if (dist < slapRange) {
                        ...
                        if (p.hits >= 3) {
                            ...
                            gameData.coins += 5;
                            score += 50;
                            addFloatText('🪅 BREAK! +5💰', p.x, p.y);

                            // Drop heart
                            spawnPowerup('heart', p.x, p.y);
            ...
            function spawnPinata() {
                ...
                pinatas.push({ x, y, vy: 50, hits: 0, w: 40, h: 40, offset: Math.random() * 100 });
            }
            ...
            function updatePinatas(dt) {
                for (let i = pinatas.length - 1; i >= 0; i--) {
                    ...
                    p.vy += 80 * dt; // Gravity
                    p.y += p.vy * dt;
                    p.x += Math.sin(timeElapsed * 3 + p.offset) * 40 * dt; // Sway
            ...
            function drawPinatas() {
                pinatas.forEach(p => {
                    ctx.save();
                    ctx.translate(p.x, p.y);
                    // Add shake if hit recently (simulated by hit count and slight random offset if moving up)
                    if (p.vy < 0) {
                        ctx.translate((Math.random() - 0.5) * 4, (Math.random() - 0.5) * 4);
                    }
                    ctx.font = '40px "Noto Color Emoji", "Apple Color Emoji", "Segoe UI Emoji", sans-serif';
                    ctx.textAlign = 'center';
                    ctx.textBaseline = 'middle';
                    ctx.fillText('🪅', 0, 0);
                    ctx.restore();
