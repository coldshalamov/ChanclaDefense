        (() => {
            ...
            let gameData = { coins: 0, upgrades: { lives: 0, shield: 0, cooldown: 0, speed: 0 }, bestScore: 0, stats: { totalSlaps: 0, perfectSlaps: 0, gamesPlayed: 0, totalCoinsEarned: 0, wins: 0 }, achievements: {}, cosmetics: ['none'], currentHat: 'none' };
            try {
                ...
                if (!gameData.stats) gameData.stats = { totalSlaps: 0, perfectSlaps: 0, gamesPlayed: 0, totalCoinsEarned: 0, wins: 0 };
                if (gameData.stats.wins === undefined) gameData.stats.wins = 0;
            ...
            const dialogue = {
                ...
                petOwen: [
                    "Ay, Alecito te protege solo por hojuelas de salmón, qué suerte tienes.",
                    "Mira, hasta Owen te defiende de mis chanclas.",
                    "Si sobrevives es por Owen, no por tus reflejos, ¿eh?"
                ],
                petRita: [
                    "Rita Cat-Alina dice miau y te regala una vida. 🐱",
                    "La reina Rita canceló una chancla por ti, da las gracias.",
                    "Hasta la gata te cuida… yo solo te aviento chancla."
                ],
                win: [
            ...
            function resetGame() {
                ...
                player.hitTimer = 0;
                isa.maxAnger = 100 + (gameData.stats.wins * 20);
                ...
                score = 0;
                comboCount = 0;
                spawnInterval = Math.max(0.5, 1.2 - (gameData.stats.wins * 0.05));
                baseSpeed = 140 + (gameData.stats.wins * 5);
                timeElapsed = 0;
                superEnabled = false;
                specialAttackBar = 0;
                slapCooldown = 0;
            ...
            function drawTitleScreen() {
                ...
                ctx.font = '16px sans-serif';
                ctx.fillText(`Isa vs. Su Gringo Para Siempre (Lv. ${(gameData.stats.wins || 0) + 1})`, canvas.width / 2, 330);
                ctx.font = '14px sans-serif';
            ...
            function spawnChancla() {
                ...
                const isFire = isa.enraged && Math.random() < 0.25;
                const w = isSuper ? 46 : 32;
                const h = isSuper ? 26 : 18;
                const x = 40 + Math.random() * (canvas.width - 80);
                const y = isa.y + 40;
                const vy = baseSpeed + Math.random() * 60 + (isSuper ? 40 : 0) + (isFire ? 80 : 0);
            ...
            function applyPowerup(p) {
                if (p.kind === 'heart') {
                    if (player.lives < player.maxLives) {
                        player.lives++;
                        addFloatText('I love you! +1 ❤️', p.x, p.y);
                    } else {
                        addFloatText('Full corazón', p.x, p.y);
                    }
                }
                if (p.kind === 'beer') {
                    slowEffect.timer = 4.5;
                    addFloatText('Cerveza chill 🍺', p.x, p.y);
                }
            ...
            function trySlap() {
                ...
                if (slappedAny) {
                    ...
                    slapCooldown = 0.3 - 0.015 * cdLevel;
                } else {
                    slapCooldown = 0.15;
                    comboCount = 0;
                    playWhoosh();
            ...
            function updateChanclas(dt) {
                ...
                for (let i = chanclas.length - 1; i >= 0; i--) {
                    ...
                    if (c.slapped) {
                        ...
                        if (distToIsa < 40 + c.w / 2) {
                            ...
                            if (isa.anger <= 0) {
                                state = STATE.WIN;
                                gameData.stats.wins++;
                                const bonus = 50 + (gameData.stats.wins * 50);
                    ...
                    if (!c.slapped && rectsOverlap(player, c)) {
                        ...
                        comboCount = 0;
                        const shakeAmt = c.type === 'super' ? 15 : (c.type === 'fire' ? 15 : 8);
            ...
            function drawWinScreen() {
                ...
                if (winProgress > 0.7) {
                    ...
                    ctx.textAlign = 'center';
                    ctx.textBaseline = 'middle';
                    const winMsg = dialogueText || "Te amo, Papi. I only hit you because I love you";
                    wrapText(ctx, winMsg, canvas.width / 2, bubbleY + 25, bubbleWidth - 14, 16);

                    ctx.fillStyle = '#ff9f1c';
                    ctx.font = 'bold 14px sans-serif';
                    const bonus = 50 + (gameData.stats.wins * 50);
                    ctx.fillText(`Boss Defeated! Coins Earned: ${bonus}`, canvas.width / 2, bubbleY + 55);
                }
