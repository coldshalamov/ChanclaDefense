        ...
        (() => {
            ...
            const isa = { x: canvas.width / 2, y: 70, anger: 100, maxAnger: 100, hitTimer: 0 };
            ...
            function update(dt) {
                ...
                if (player.dashTimer > 0) {
                    player.dashTimer -= dt;
                    player.x += speed * 3.5 * player.dashDir * dt;
                    if (Math.random() < 0.3) {
                        dashTrails.push({ x: player.x, y: player.y, timer: 0.2, dir: player.dashDir });
                    }
                } else {
            ...
            function drawDashTrails() {
                dashTrails.forEach(t => {
                    ctx.save();
                    ctx.globalAlpha = Math.max(0, t.timer / 0.2) * 0.5;
            ...
            function handleKey(e, isDown) {
                ...
                if (e.key === 'Shift' && isDown && state === STATE.PLAYING) {
                    if (player.dashCooldown <= 0) {
                        player.dashTimer = 0.2;
                        player.dashCooldown = 1.0;
                        player.dashDir = keys.left ? -1 : (keys.right ? 1 : (Math.random() < 0.5 ? -1 : 1));
                        playSound(300, 0.1);
                        dashTrails.push({ x: player.x, y: player.y, timer: 0.2, dir: player.dashDir });
                    }
                    e.preventDefault();
            ...
            function handleMobileZones(zone, dir) {
                zone.addEventListener('touchstart', (e) => {
                    ...
                    const now = Date.now();
                    const lastTap = dir === 'left' ? lastTapLeft : lastTapRight;
