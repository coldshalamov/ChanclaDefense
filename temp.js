        (() => {
            ...
            let gameData = { coins: 0, upgrades: { lives: 0, shield: 0, cooldown: 0, speed: 0 }, bestScore: 0, stats: { totalSlaps: 0, perfectSlaps: 0, gamesPlayed: 0, totalCoinsEarned: 0 }, achievements: {}, cosmetics: ['none'], currentHat: 'none' };
            try {
                const saved = localStorage.getItem('chancla_bomb_save');
                if (saved) {
                    gameData = JSON.parse(saved);
                    // Backward compatibility for old boolean saves and missing speed
            ...
            const isa = { x: canvas.width / 2, y: 70, anger: 100, maxAnger: 100, hitTimer: 0, enraged: false };

            const COSMETICS_LIST = [
                { id: 'none', name: 'No Hat', icon: '', cost: 0 },
                { id: 'cap', name: 'Baseball Cap', icon: '🧢', cost: 100 },
                { id: 'cowboy', name: 'Cowboy Hat', icon: '🤠', cost: 250 },
                { id: 'crown', name: 'Crown', icon: '👑', cost: 500 },
                { id: 'party', name: 'Party Hat', icon: '🥳', cost: 150 },
                { id: 'tophat', name: 'Top Hat', icon: '🎩', cost: 300 }
            ];

            const ACHIEVEMENTS_LIST = [
                { id: 'first_slap', name: 'First Slap', desc: 'Slap your first chancla', reward: 10, check: () => gameData.stats.totalSlaps >= 1 },
                { id: 'slap_master', name: 'Slap Master', desc: 'Slap 500 chanclas', reward: 100, check: () => gameData.stats.totalSlaps >= 500 },
                { id: 'perfect_timing', name: 'Perfect Timing', desc: 'Get 50 perfect slaps', reward: 50, check: () => gameData.stats.perfectSlaps >= 50 },
                { id: 'high_roller', name: 'High Roller', desc: 'Score 1000 in one game', reward: 150, check: () => gameData.bestScore >= 1000 },
                { id: 'rich_gringo', name: 'Rich Gringo', desc: 'Earn 1000 total coins', reward: 200, check: () => gameData.stats.totalCoinsEarned >= 1000 },
                { id: 'veteran', name: 'Veteran', desc: 'Play 50 games', reward: 75, check: () => gameData.stats.gamesPlayed >= 50 }
            ];
            ...
            const dialogue = {
                start: [
                    ...
                    "¡Ya sé que lo hiciste en la ducha sin grabarlo para mí!",
                ...
                hitNormal: [
                    "¡Eso te pasa por andar dando like a otras, eh! 😈",
                    "Te dije, mi vida, la chancla siempre encuentra tu cara.",
                    "Ay, gringo, ni esquivar sabes… ven pa’ acá.",
                    "Mira nomás, directo en tu carita bonita, jajaja.",
                    "¡Eso te pasa por andar dando like a otras, eh! 😈",
                    "Te dije, mi vida, la chancla siempre encuentra tu cara.",
                    "Ay, gringo, ni esquivar sabes… ven pa’ acá.",
                    "Mira nomás, directo en tu carita bonita, jajaja.",
                ...
                hitSuper: [
                    "Esta es la de 15 centímetros, para que respetes a tu reina. 💥",
                    "Chancla XL activada… F por tu carita, mi vida.",
                    "Te advertí de la chancla deluxe, ¿no?"
                ],
                nearMiss: [
                    "Te salvaste por las pestañas, ¿eh?",
                ...
                lowHealth: [
                    "¡NUNCA VES MIS REELS!",
                    "Última vida, mi amor… igual que mi paciencia cuando me dejas en visto.",
                    "Ya casi te mando directo a ‘Game Over, mi vida’."
                ],
                gameOver: [
                    "Isa ganó, como siempre. 😏",
                    "Game over, mi gringo. ¿Otra vez o ya te rindes?",
                    "Te lo dije: con chancla no se juega.",
                    "Perdiste, pero igual te amo poquito… muy poquito."
                ],
                highScore: [
                    "Ok, estoy impresionada… sí sabes esquivar, ¿eh?",
                    "Mi gringo pro player, qué miedo y qué ganas.",
                    "Está bien… hoy guardo la chancla. Pero solo hoy."
                ],
                win: [
                    "Te amo, Papi. I only hit you because I love you."
                ],
                slapSuccess: [
                    "¿Cómo te atreves a manotear mis chanclas?!",
            ...
            function resetGame() {
                ...
                player.speed = 230 + 15 * (gameData.upgrades.speed || 0);
            ...
            function getComboPhrase(c) {
                if (c >= 25) return "LEGEND! 👑";
                if (c >= 15) return "IMPOSSIBLE! 🤯";
                if (c >= 10) return "DIOS MIO! 😱";
                if (c >= 6) return "FUEGO! 🔥";
                if (c >= 3) return "SPICY! 🌶️";
                return "";
            }
            ...
            function drawBackground() {
                ...
                if (slowMoTimer > 0) {
                    ctx.save();
                    ctx.fillStyle = 'rgba(0, 229, 255, 0.15)'; // Cyan slow-mo tint
                    ctx.fillRect(0, 0, canvas.width, canvas.height);
                    ctx.restore();
                }

                ctx.fillStyle = isa.enraged ? '#1a0508' : '#0a1527';
            ...
            function drawDashTrails() {
                dashTrails.forEach(t => {
                    ctx.save();
                    ctx.globalAlpha = Math.max(0, t.life / 0.15) * 0.5;
                    ctx.filter = 'drop-shadow(0 0 5px #00ffff) drop-shadow(0 0 10px #00ffff) sepia(1) hue-rotate(140deg) saturate(3)';
                    drawPlayerAvatar(ctx, t.x, t.y, Math.min(player.w, player.h), false, 'normal');
                    ctx.restore();
                });
            }
            ...
            function drawPlayerAvatar(ctx, x, y, size, hasRose, expression = 'normal') {
                ...
                // Draw Hat
                const hat = COSMETICS_LIST.find(h => h.id === gameData.currentHat);
                if (hat && hat.icon) {
                    ctx.save();
                    ctx.font = '36px Noto Color Emoji, Apple Color Emoji, Segoe UI Emoji, sans-serif';
                    ctx.textAlign = 'center';
                    ctx.textBaseline = 'bottom';
                    ctx.fillText(hat.icon, 0, -headH / 2 + 10);
                    ctx.restore();
                }
                ...
                if (expression === 'cocky') {
                    ...
                    ctx.textAlign = 'center';
                    ctx.textBaseline = 'middle';
                    ctx.fillText('✨', headW * 0.6, -headH * 0.4);
                }
            ...
            function drawHUD() {
                ...
                ctx.font = '14px sans-serif';
                ctx.fillText('💰 ' + gameData.coins, canvas.width / 2, 16);

                ctx.textAlign = 'right';
                ctx.fillStyle = '#fff';
                ctx.font = '16px sans-serif';
                ctx.fillText('Lives: ' + '❤️'.repeat(player.lives), canvas.width - 12, 22);
                ...
                if (p >= 1) {
                    ctx.save();
                    ctx.translate(barX + barWidth / 2, barY + barHeight + 20);
            ...
            function drawAchievements() {
                ...
                ctx.fillText('LOGROS / ACHIEV.', canvas.width / 2, 60);

                ctx.fillStyle = '#ffd700';
                ctx.font = '24px sans-serif';
                ctx.fillText('💰 ' + gameData.coins, canvas.width / 2, 95);

                let y = 130;
                ACHIEVEMENTS_LIST.forEach(a => {
                    const status = gameData.achievements[a.id];
                    const unlocked = a.check();

                    let bgColor = '#555'; // Locked
                    if (status === 'claimed') bgColor = '#4caf50';
                    else if (unlocked) bgColor = '#ff9f1c'; // Unlocked but not claimed

                    // Button bg
                    ctx.fillStyle = bgColor;
                    roundRect(ctx, 30, y, canvas.width - 60, 65, 10);
                    ctx.fill();
                    ...
                    if (status === 'claimed') {
                        ctx.fillText('CLAIMED', canvas.width - 45, y + 35);
                    } else if (unlocked) {
                        ctx.fillText('CLAIM 💰 ' + a.reward, canvas.width - 45, y + 35);
                    } else {
                        ctx.fillText('LOCKED 💰 ' + a.reward, canvas.width - 45, y + 35);
                    }
            ...
            function drawShop() {
                ...
                ctx.textAlign = 'center';
                ctx.font = 'bold 32px sans-serif';
                ctx.fillText('SHOP / TIENDA', canvas.width / 2, 80);
            ...
            function drawCosmetics() {
                ...
                ctx.textAlign = 'center';
                ctx.font = 'bold 32px sans-serif';
                ctx.fillText('COSMETICS', canvas.width / 2, 80);
                ...
                COSMETICS_LIST.forEach(c => {
                    const unlocked = gameData.cosmetics.includes(c.id) || c.cost === 0;
                    const equipped = gameData.currentHat === c.id;
                    const affordable = gameData.coins >= c.cost;

                    let bgColor = '#555';
                    if (equipped) bgColor = '#4caf50';
                    else if (unlocked) bgColor = '#2196f3';
                    ...
                    ctx.textAlign = 'left';
                    ctx.fillStyle = '#fff';
                    ctx.fillText(c.icon || '👤', 55, y + 45);

                    // Name
                    ctx.font = 'bold 18px sans-serif';
                    ctx.fillText(c.name, 100, y + 42);
                    ...
                    if (equipped) {
                        ctx.fillStyle = '#fff';
                        ctx.fillText('EQUIPPED', canvas.width - 55, y + 42);
                    } else if (unlocked) {
                        ctx.fillStyle = '#fff';
                        ctx.fillText('EQUIP', canvas.width - 55, y + 42);
                    } else {
                        ctx.fillStyle = affordable ? '#ffd700' : '#ff9999';
                        ctx.fillText('💰 ' + c.cost, canvas.width - 55, y + 42);
                    }
            ...
            function drawTitleScreen() {
                ...
                roundRect(ctx, 110, 440, canvas.width - 220, 46, 12);
                ctx.fill();
                ctx.fillStyle = '#fff';
                ctx.font = '18px sans-serif';
                ctx.fillText('Shop / Tienda', canvas.width / 2, 470);

                // Achievements Button
                ctx.fillStyle = '#9c27b0';
                roundRect(ctx, 110, 500, canvas.width - 220, 46, 12);
                ctx.fill();
                ctx.fillStyle = '#fff';
                ctx.font = '18px sans-serif';
                ctx.fillText('Logros / Achiev.', canvas.width / 2, 530);
            ...
            function drawGameOver() {
                ...
                ctx.fillText('Isa ganó', canvas.width / 2, canvas.height / 2 + 10);

                ctx.font = '20px sans-serif';
                ctx.fillText('Score: ' + score + ' | Best: ' + gameData.bestScore, canvas.width / 2, canvas.height / 2 + 45);

                ctx.fillStyle = '#ffd700';
                ctx.font = '18px sans-serif';
                const earned = Math.floor(score / 10);
                ctx.fillText(`Coins Earned: +${earned}`, canvas.width / 2, canvas.height / 2 + 70);
                ctx.font = '14px sans-serif';
                ctx.fillText(`Total Coins: ${gameData.coins}`, canvas.width / 2, canvas.height / 2 + 90);
            ...
            function spawnMeteor() {
                ...
                const vx = 0;
                chanclas.push({
                    x, y, vx, vy, w, h, type: 'meteor', rotation: 0, rotSpeed: 0.2,
                    slapsNeeded: 12, currentSlaps: 0, isMeteor: true, dodged: false
                });
                addFloatText('¡LA CHANCLA GIGANTE!', canvas.width / 2, 120);
                playSound(1000, 0.4);
                triggerShake(15, 1.5);
            }
            ...
            function spawnChancla() {
                ...
                const isSuper = superEnabled && !isBomb && Math.random() < 0.18;
                const isFire = isa.enraged && !isBomb && !isSuper && Math.random() < 0.25;
                ...
                chanclas.push({ x, y, vx, vy, w, h, type, rotation: 0, rotSpeed, dodged: false });
            ...
            function trySlap() {
                ...
                for (let i = pinatas.length - 1; i >= 0; i--) {
                    const p = pinatas[i];
                    ...
                    if (dist < slapRange) {
                        ...
                        p.hits++;
                        p.vy = -400; // Bounce up
                        p.vx += (p.x - player.x) * 5; // Impart momentum
                        ...
                        if (p.hits >= 3) {
                            // Pop piñata
                            playSound(600, 0.2);
                            addFloatText('PIÑATA POP! +20💰', p.x, p.y - 20);
                            score += 50;
                            gameData.coins += 20;
                            gameData.stats.totalCoinsEarned += 20;
                            ...
                            for (let k = 0; k < 15; k++) {
                                rosePetals.push({
                                    ...
                                    rotation: Math.random() * 6, rotSpeed: (Math.random() - 0.5) * 10,
            ...
            function fireSpecialAttack() {
                if (specialAttackBar >= maxSpecialAttack) {
                    ...
                    addFloatText('SPECIAL! 🦵', player.x, player.y - 30);
            ...
            function updatePinatas(dt) {
                ...
                for (let i = pinatas.length - 1; i >= 0; i--) {
                    ...
                    if (p.x > canvas.width - 20) { p.x = canvas.width - 20; p.vx *= -0.8; }
                    ...
                    if (!c.slapped && c.type !== 'meteor' && rectsOverlap(player, c)) {
                        if (dashTimer > 0) {
                            if (!c.dodged) {
                                c.dodged = true;
                                slowMoTimer = 1.5;
                                addFloatText('PERFECT DODGE! ⏱️', player.x, player.y - 40);
                                triggerFlash(0.2, '#00e5ff');
                                playSound(1200, 0.3);
                                spawnImpact(player.x, player.y, true);
                            }
            ...
            function updateSpecialProjectiles(dt) {
                for (let i = specialProjectiles.length - 1; i >= 0; i--) {
                    ...
                    if (dist < 15) {
                        ...
                        addFloatText(word + ' ❤️', isa.x, isa.y);
            ...
            function update(dt) {
                ...
                if (isa.anger <= isa.maxAnger * 0.3 && !isa.enraged) {
                    isa.enraged = true;
                    // Rage phase entry effects
                    triggerShake(10, 0.5);
                    triggerFlash(0.3, '#ff4d4d');
                    playSound(1000, 0.3);
                    addFloatText('¡ENRAGED!', isa.x, isa.y - 30);
                }

                if (slowMoTimer > 0) {
                    slowMoTimer -= dt; // Real dt
                    dt *= 0.3; // Scale game dt
                }
                timeElapsed += dt;
                spawnTimer += dt;
                if (timeElapsed > 12) {
                    // Ramp up difficulty faster if enraged
                    const intervalDec = isa.enraged ? 0.05 : 0.02;
                    const speedInc = isa.enraged ? 10 : 5;

                    spawnInterval = Math.max(minSpawnInterval, spawnInterval - dt * intervalDec);
                    baseSpeed += dt * speedInc;
                }
                ...
                if (isa.enraged) {
                    meteorTimer += dt;
                    if (meteorTimer >= 12 && !chanclas.some(c => c.type === 'meteor')) {
                        spawnMeteor();
                        meteorTimer = 0;
                    }
                }
                ...
                if (isa.enraged) {
                    if (Math.random() < 0.3) {
                        fireParticles.push({
                            x: isa.x + (Math.random() - 0.5) * 60,
                            y: isa.y + (Math.random() - 0.5) * 60,
            ...
            canvas.addEventListener('click', (e) => {
                ...
                if (state === STATE.TITLE) {
                    ...
                    // Check Shop Button (110, 440, w, 46)
                    else if (pos.y >= 440 && pos.y <= 486 && pos.x >= 110 && pos.x <= canvas.width - 110) {
                        setDirectionsVisible(false);
                        state = STATE.SHOP;
                    }
                    // Check Achievements Button (110, 500, w, 46)
                    else if (pos.y >= 500 && pos.y <= 546 && pos.x >= 110 && pos.x <= canvas.width - 110) {
                        setDirectionsVisible(false);
                    ...
                    let y = 130;
                    for (let a of ACHIEVEMENTS_LIST) {
                        if (pos.y >= y && pos.y <= y + 65 && pos.x >= 30 && pos.x <= canvas.width - 30) {
                            const status = gameData.achievements[a.id];
                            const unlocked = a.check();
                            if (unlocked && status !== 'claimed') {
                                gameData.coins += a.reward;
                                gameData.achievements[a.id] = 'claimed';
                                saveGameData();
                                playSound(1200, 0.1); // Cha-ching!
                            }
                        }
                    ...
                    let y = 140;
                    for (let c of COSMETICS_LIST) {
                        if (pos.y >= y && pos.y <= y + 70 && pos.x >= 40 && pos.x <= canvas.width - 40) {
                            const unlocked = gameData.cosmetics.includes(c.id) || c.cost === 0;
                            if (unlocked) {
                                gameData.currentHat = c.id;
                                saveGameData();
                                playSound(900, 0.1);
                            } else if (gameData.coins >= c.cost) {
                                gameData.coins -= c.cost;
                                gameData.cosmetics.push(c.id);
                                gameData.currentHat = c.id;
                                saveGameData();
                                playSound(1200, 0.1); // Cha-ching!
                            } else {
                    ...
                    // Play Again Button (110, 550, w, 50)
                    if (pos.y >= 550 && pos.y <= 600 && pos.x >= 110 && pos.x <= canvas.width - 110) {
                        resetGame();
                    } else if (pos.y > 600) { // Catch-all for bottom clicks just in case
                         resetGame();
                    }
                }
            });
            ...
            canvas.addEventListener('touchstart', (e) => {
                ...
                const scaleY = canvas.height / rect.height;
                const pos = {
                    x: (touchPoint.clientX - rect.left) * scaleX,
                    y: (touchPoint.clientY - rect.top) * scaleY
                };

                if (state === STATE.TITLE) {
                    if (pos.y >= 380 && pos.y <= 426 && pos.x >= 110 && pos.x <= canvas.width - 110) startGameFromTitle();
                    else if (pos.y >= 440 && pos.y <= 486 && pos.x >= 110 && pos.x <= canvas.width - 110) {
                        setDirectionsVisible(false);
                        state = STATE.SHOP;
                    }
                ...
                if (state === STATE.PLAYING) {
                    const relativeY = pos.y / canvas.height;
            ...
            function handleKey(e, isDown) {
                if (['ArrowLeft', 'a', 'A'].includes(e.key)) {
                    if (isDown && !keys.left && !e.repeat && dashCooldown <= 0) {
                        if (timeElapsed - lastTapLeft < 0.25) {
                            dashTimer = 0.15;
                            dashCooldown = 0.8;
                            dashDir = -1;
                        }
                        lastTapLeft = timeElapsed;
                    }
                    keys.left = isDown;
                    e.preventDefault();
                }
                if (['ArrowRight', 'd', 'D'].includes(e.key)) {
                    if (isDown && !keys.right && !e.repeat && dashCooldown <= 0) {
                        if (timeElapsed - lastTapRight < 0.25) {
                            dashTimer = 0.15;
                            dashCooldown = 0.8;
                            dashDir = 1;
                        }
                        lastTapRight = timeElapsed;
                    }
                    keys.right = isDown;
                    e.preventDefault();
                }
