...
def verify_damage():
    ...
    with sync_playwright() as p:
        ...
        # Call directly special projectiles hit to see if we can trigger the -20 float text
        page.evaluate("""
            ...
            if (allC.length > 0) {
                ...
                projectile.type = 'normal'; // Use normal so it doesn't trigger BOOM which might occlude
            }
        """)

        time.sleep(0.05) # Wait for update loop to process collision
        page.screenshot(path='/app/verification/damage_numbers2.png')
